...
An Amazon Ads session must already be signed in within the Agent's Chrome profile. The endpoint uses the same `X-Api-Key` as the rest of the Agent. It does not accept cookies or authentication fields from the Desktop client. The supplied response example contains one complete page; pagination with more than 50 campaigns still requires a live Amazon Ads check.
