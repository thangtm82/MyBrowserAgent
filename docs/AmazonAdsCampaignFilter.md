...
The response follows the usual `ApiResult` envelope. `Data` is the array from `report.data`; each campaign retains all fields provided by Amazon Ads, including `campaignId`, `campaignName`, `acos`, `spend`, and `sales`. A successful filter with no matches returns `[]`. The Agent follows the `report.tokenPagination.nextPageToken` continuation returned by Amazon Ads (with fallback support for `nextToken` and `token`). If Amazon Ads does not return a continuation token, the Agent returns the rows in `report.data`; `numberOfRecords` alone is not used to infer that another page exists.
